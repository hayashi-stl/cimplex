...
impl<C: Clone, I: Iterator, U: IntoIterator, F: FnMut(C, I::Item) -> U> Iterator
    for FlatMapWith<C, I, U, F>
{
    type Item = U::Item;
        ...
        loop {
            if let Some(item) = self.inner_iter.as_mut().and_then(|iter| iter.next()) {
                ...
                self.inner_iter = self
                    .iter
                    .next()
                    .map(|t| (self.combine)(self.common.clone(), t).into_iter());
...
impl<
        V: Clone + Eq + Hash,
        NI: IntoIterator<Item = V>,
        NF: FnMut(&V) -> NI,
        SF: FnMut(&V) -> bool,
    > Iterator for Bfs<V, NF, SF>
{
    type Item = V;
...
mod tests {
    ...
    #[test]
    fn test_bfs_empty() {
        let bfs = bfs(vec![0, 4], |i| vec![i + 1, i + 2], |i| *i < 0);
        assert_eq!(bfs.collect::<Vec<_>>(), vec![]);
    }

    #[test]
    fn test_bfs() {
        let bfs = bfs(vec![0, 4], |i| vec![i + 1, i + 2], |i| *i < 8);
        assert_eq!(bfs.collect::<Vec<_>>(), vec![0, 4, 1, 2, 5, 6, 3, 7]);
    }
}
